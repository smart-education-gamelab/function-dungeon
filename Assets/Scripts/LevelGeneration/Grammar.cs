...
public class Grammar {
    ...
    public int height = 0;
    [Range(0.0f, 1.0f)]
    public float chanceToSkip = 0.0f;
    public string pattern = "";
    ...
    */
    public void GenerateVariations() {
        if (!enabled) return;
        if (width > LevelGenerator.maxGrammarSize * 2 || height > LevelGenerator.maxGrammarSize * 2) Debug.Log($"Grammar [{name}] size is bigger than max grammar size as specified in the level generator script. This will break this grammar.");
